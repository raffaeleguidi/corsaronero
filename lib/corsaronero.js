module.exports = {
    ...
    latestAdditionsFor: function(mediaType, cb) {
        latestAdditionsFor(mediaType, cb);
    },
    latestAdditions: function(cb) {
        latestAdditions(cb);
    },
...
function latestAdditionsFor(mediaType, cb) {
    var results = new Array();
    request('http://ilcorsaronero.info/cat/' + mediaType, function (error, response, body) {
...
function latestAdditions(cb) {
    var results = new Array();
    var mediaType = ["game", "movie", "music", "series"];
    request('http://ilcorsaronero.info', function (error, response, body) {
        if (!error && response.statusCode == 200) {
            $ = cheerio.load(response.body);
            var sizes=$('font[size="-2"][color="#FF6600"]');
            var dates=$('font[color="#669999"]');
            ...
            $('a.tab').each(function(i, elem) {
                ...
                var size = $(sizes.get(i)).text();
                var date = $(dates.get(i)).text();
                var type = mediaType[parseInt(n++ / 7)];
                var seeds = $($(row).find("td").get(5)).text();
                var leeches = $($(row).find("td").get(6)).text();
                var item = {
                    filename: filename,
